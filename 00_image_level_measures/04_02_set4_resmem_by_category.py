"""
Applies the ResMem model to compute memorability predictions for a single category in the SUN397 dataset.

Requires:
- SUN397 images organized by category in:
    ../data/stimuli/SUN397/
- Pretrained ResMem model and associated image transformer (https://github.com/Brain-Bridge-Lab/resmem).

Usage:
    python 04_02_set4_resmem_by_category.py <idx>
    (where <idx> is the zero-based index of the category to process)

Outputs:
- CSV file with ResMem memorability predictions for the specified category:
    ../data/image_level_measure/set4_by_category/set4_{category_name}_resmem.csv
"""
...
def main(idx):
    ...
    df.to_csv(os.path.join('..', 'data', 'image_level_measure', 'set4_by_category', f'set4_{renamed_category}_resmem.csv'), index=False)
